...
class Event extends CI_Model{ 
	...
	function getEvent($year, $month, $day){
		...
		$query = $this->db->select(TBL_TIMETABLE_TRANSECTIONS.'.id as id,'.TBL_TIMETABLE_TRANSECTIONS.'.timings as time,'.TBL_TIMETABLE_TRANSECTIONS.'.topic as event,'.TBL_USER.'.name as trainer,a.name as backup_trainername')
		                     ->from(TBL_TIMETABLE)
							 ->join(TBL_TIMETABLE_TRANSECTIONS,TBL_TIMETABLE.'.id = '.TBL_TIMETABLE_TRANSECTIONS.'.time_table_id')
