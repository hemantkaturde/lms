...
class Event extends CI_Model{ 
	...
	function getDateEvent($year, $month){
		...
		$query = $this->db->select(TBL_TIMETABLE_TRANSECTIONS.'.date as event_date, count(time_table_id) as total_events ')->from(TBL_TIMETABLE)->join(TBL_TIMETABLE_TRANSECTIONS,TBL_TIMETABLE.'.id = '.TBL_TIMETABLE_TRANSECTIONS.'.time_table_id')->like(TBL_TIMETABLE_TRANSECTIONS.'.date', $year, 'after')->group_by(TBL_TIMETABLE_TRANSECTIONS.'.date')->get();
