...
class Api_model extends CI_Model
{
    ...
    public function getEnquiryData($data){
        ...
        try{
            ...
            $this->db->select('*,'.TBL_ADMISSION.'.enq_id as admissionexits,'.TBL_ENQUIRY.'.enq_id as id,'.TBL_ENQUIRY.'.enq_id as enq_id,'.TBL_ADMISSION.'.enq_id as admission_status,'.TBL_ENQUIRY.'.enq_number,'.TBL_USER.'.name as counseller,'.TBL_ENQUIRY.'.enq_fullname,'.TBL_ENQUIRY.'.enq_mobile,'.TBL_ENQUIRY.'.enq_email,'.TBL_ENQUIRY.'.doctor_non_doctor,'.TBL_ENQUIRY.'.enq_course_id,'.TBL_CITIES.'.name city_name,'.TBL_ENQUIRY.'.counsellor_id,'.TBL_ENQUIRY.'.payment_status as ps');
    ...
    public function getaddoncoursepaymentdetails($id){
        ...
        if(count($fetch_result) > 0)
        {
            foreach ($fetch_result as $key => $value)
            {
                ...
                $data[$counter]['invoice_link'] = "https://iictn.in/tax_invoice/index.php?enq_id=".$value['enquiry_id']."&paymentid=".$value['paymentid'];
    ...
    public function gettimetabledetailslist($timetable_id,$user_flag,$userid){
        ...
        if(count($fetch_result) > 0)
        {
            foreach ($fetch_result as $key => $value)
            {
                 ...
                 $data[$counter]['to_date'] = date('d-m-Y', strtotime($value['to_date']));
                 $data[$counter]['class_date'] = date('d-m-Y', strtotime($value['date']));
                 $data[$counter]['timings'] = $value['timings'].' |  Duration '.$value['total_hrs'].' hrs';
    ...
    public function studentexamrequest($user_flag,$userid){
      ...
      public function getalltotalrevunedetailscounsellor($userid,$user_flag)
      {
        ...
        if($this->admission_model->total_discount_add_on_consellor($userid,$user_flag)[0]['total_discount']!=null){
            $dataAddoncourse['total_discount'] = $this->admission_model->total_discount_add_on_consellor($userid,$user_flag)[0]['total_discount'];
        }else{
            $dataAddoncourse['total_discount'] = 0;
        }
       
        if($this->admission_model->total_pending_add_on_consellor($userid,$user_flag)[0]['total_pending']!=null){
            $total_pending_Add_on_single = $this->admission_model->total_pending_add_on_consellor($userid,$user_flag)[0]['total_pending'];
        }else{
            $total_pending_Add_on_single = 0;
        }
    ...
    public function getcourseliststudent($data){
        ...
        $this->db->select('enq_course_id');
        $this->db->join(TBL_ENQUIRY, TBL_ENQUIRY.'.enq_id = '.TBL_USERS_ENQUIRES.'.enq_id');
        $this->db->where(TBL_USERS_ENQUIRES.'.user_id', $data);
        $query = $this->db->get(TBL_USERS_ENQUIRES);
        $fetch_result_enquiry_courses = $query->result_array();
        ...
        $counter = 0;
         ...
         $course_ids    =   explode(',', $value['enq_course_id']);
            ...
            foreach ($course_ids as $key => $value) {
                  ...
                  $data[$counter]['mainCourse'] = $getCourseInfo[0]->mainCourse_condition;
                  $data[$counter]['sponsored'] = $getCourseInfo[0]->sponsored_condition;
                  $data[$counter]['regular'] = $getCourseInfo[0]->regular_condition;
