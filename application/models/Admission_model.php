...
class Admission_model extends CI_Model
{
                ...
                //  $data[$counter]['row-index'] = 'row_'.$value['courseId'];
                    ...
                    // $data[$counter]['address'] = $value['address'];
                     ...
                     if($value['cancle_status']==1){
                        ...
                        $data[$counter]['action'] .= "<a style='cursor: pointer;'  href='admissioncancleinfo/index.php?admission_id=".$value['id']."' target='_blank'  class='print_certificate' data-id='".$value['id']."'><img width='20' src=".ICONPATH."/print.png alt='Print Admission Cancellation form' title='Print Admission Cancellation form'></a> "; 
    ...
    public function upcoming_class_links($userId){
        ...
        if(count($fetch_result) > 0)
        {
            foreach ($fetch_result as $key => $value)
            {
                    ...
                    $data[$counter]['courseId'] = $value['courseId'];
                    $data[$counter]['courserestrictId'] = $check_course_restricted_or_not[0]['restrict'];
    ...
    public function getAttendancedata($params){
        ...
        $this->db->select('*,'.TBL_TIMETABLE_TRANSECTIONS.'.total_hrs as totalcalulatedhrs');
        $this->db->join(TBL_COURSE, TBL_COURSE.'.courseId = '.TBL_ATTENDANCE.'.course_id');
        $this->db->join(TBL_USER, TBL_USER.'.userId  = '.TBL_ATTENDANCE.'.user_id');
        $this->db->join(TBL_TIMETABLE_TRANSECTIONS, TBL_TIMETABLE_TRANSECTIONS.'.id = '.TBL_ATTENDANCE.'.topic_id');
        // $this->db->join(TBL_TOPIC_MEETING_LINK, TBL_TOPIC_MEETING_LINK.'.id = '.TBL_ATTENDANCE.'.meeting_id');
        ...
        if(count($fetch_result) > 0)
        {
            foreach ($fetch_result as $key => $value)
            {
                //  $data[$counter]['row-index'] = 'row_'.$value['courseId'];
                 ...
                 if($value['totalcalulatedhrs']){
                    $data[$counter]['total_hours'] = $value['totalcalulatedhrs'].' hours';
                 }else{
                    $data[$counter]['total_hours'] = '';
                 }
    ...
    public function getAttendancereportdata($params,$search_by_student,$from_date,$to_date){
        ...
        $this->db->select('*,'.TBL_TIMETABLE_TRANSECTIONS.'.total_hrs as total_hrs_calculated');
        $this->db->join(TBL_COURSE, TBL_COURSE.'.courseId = '.TBL_ATTENDANCE.'.course_id');
        $this->db->join(TBL_USER, TBL_USER.'.userId  = '.TBL_ATTENDANCE.'.user_id');
        $this->db->join(TBL_TIMETABLE_TRANSECTIONS, TBL_TIMETABLE_TRANSECTIONS.'.id = '.TBL_ATTENDANCE.'.topic_id');
        // $this->db->join(TBL_TOPIC_MEETING_LINK, TBL_TOPIC_MEETING_LINK.'.id = '.TBL_ATTENDANCE.'.meeting_id');
        ...
        if(count($fetch_result) > 0)
        {
            foreach ($fetch_result as $key => $value)
            {
                //  $data[$counter]['row-index'] = 'row_'.$value['courseId'];
                 ...
                 $data[$counter]['course_name'] = $value['course_name'];
                 $data[$counter]['topic'] = $value['topic'];
                 $data[$counter]['total_hours'] = $value['total_hrs_calculated'].' hours';
                 $data[$counter]['class_date'] =  date('d-m-Y', strtotime($value['date']));
                 //$data[$counter]['enq_date'] = date('d-m-Y', strtotime($value['enq_date']));
                 $data[$counter]['timings'] = $value['timings'];
