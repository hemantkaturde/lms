		...
		$(document).on('click','.edit_course',function(e){
			...
			$.ajax({  
                ...
                {  
					 ...
					 if(data[0].mainCourse_condition1=='Main'){
					 ...
					 if(data[0].sponsored_condition1=='Sponsored'){
						$(".sponsorededit").attr("checked", "true");
					 }else{
						//$(".course_mode_online1").attr("checked", "flase");
					 }

					 if(data[0].regular_condition1=='Regular'){
	...
	<script type="text/javascript">
		...
		function viewenquirylistreport(search_by_student,search_by_consellor,from_date,to_date){
			...
			var dt = $('#view_enquirylist_report').DataTable({
	            ...
	            "serverSide": true,
	            "ajax":{
                    url :"<?php echo base_url();?>enquiry/fetchenquiryreport/"+search_by_student+"/"+search_by_consellor+"/"+from_date+"/"+to_date,
                    type: "post",
	            },
	        });
		}
...
<?php if($pageTitle=='Tax Invoice Report'){ ?>
	    <script type="text/javascript">
				...
				$(document).on('click','#excel_export_report_tax_invoice',function(e){
					e.preventDefault();
					// $(".loader_ajax").show();
